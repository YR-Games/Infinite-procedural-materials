	...
	while file_name != "":
		if not dir.current_is_dir():
			...
			if ext in ["tres", "res", "tscn", "scn"]:
				...
				if resource:
					# Проверяем, что ресурс является MixModeData
					if resource is MixModeData:
						var key = resource.function_name
						# Если function_name пуст, используем имя файла как запасной вариант
						if key.is_empty():
							push_warning("MixModeData resource has empty function_name: ", full_path)
						...
						push_warning("Resource is not MixModeData: ", full_path)
				else:
