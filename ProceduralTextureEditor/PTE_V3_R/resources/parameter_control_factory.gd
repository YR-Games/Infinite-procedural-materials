...
static var float_double_scene = preload(
"res://PTE_V3_R/UI/float_parameter_double_ui.tscn"
)
...
static var int_double_scene = preload(
"res://PTE_V3_R/UI/int_parameter_double_ui.tscn"
)
...
static func create(def) -> ParameterControl:
	...
	if def is floatParameterDef:
		...
		match def.editor_type:
			...
			floatParameterDef.EditorType.DOUBLE:
				return float_double_scene.instantiate()
	...
	if def is intParameterDef:
		...
		match def.editor_type:
			...
			intParameterDef.EditorType.DOUBLE:
				return int_double_scene.instantiate()
